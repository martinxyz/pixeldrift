...
def generate_tile(lut, count=filter_steps):
    ...
    cells = Cells()
    cells.set_data(np.random.randint(0, 2, (size, size)))
...
def evaluate_lut(lut):
    ...
    edge_freqs = [cells.copy().apply_lut_filter(lut)
                  .get_data().astype('f').mean()
...
def main():
    ...
    for it in range(iterations):
        ...
        with open(f'best-lut-it{it:05}.txt', 'w') as f:
            np.savetxt(f, luts[0])
        cells = generate_tile(luts[0])
        t0 = time.time()
        render_cells(cells, f'best-lut-it{it:05}.png')
